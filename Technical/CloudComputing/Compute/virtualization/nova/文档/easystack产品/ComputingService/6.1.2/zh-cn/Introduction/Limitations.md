...
* 当在同一子网中创建多张网卡时，这些网卡分配IP地址的方式要保持一致。
...
* 镜像内部要安装cloud-init 0.7.9及以上的版本。

* 加密算法只支持SHA-512。

* 不支持ISO镜像创建云主机的初始密码加密。

* 被重建过的云主机、且重建之前的镜像没有配置密码加密，则之前的密码仍然会明文保存。
...
* 目前云平台支持的GPU型号如下表所示，下述所列GPU均可用于GPU透传和虚拟化切割。
...
* 云主机网卡多队列数量不随在线调整后的CPU数量变化，需硬重启云主机后生效。
